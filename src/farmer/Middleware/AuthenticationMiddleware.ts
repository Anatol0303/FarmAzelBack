...
//             next();
//             // (request as any).user = jwt.verify(jwtToken, process.env.JWT_SECRET!) as JwtPayload;
//             //
//             // console.log("Authenticated user:", (request as any).user);
//             // /next();
//         } catch (err) {
...
export class AuthenticationMiddleware implements ExpressMiddlewareInterface {
    async use(request: Request, response: Response, next: NextFunction): Promise<any> {
        ...
        try {
            // Явное указание типа с login
            request.user = jwt.verify(jwtToken, process.env.JWT_SECRET!) as JwtPayload & { login: string };
            next();
        } catch (err) {
            return response.status(403).json({ message: "Invalid token" });
        }
